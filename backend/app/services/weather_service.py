...
def fetch_current_weather(city: Optional[str] = None, lat: Optional[float] = None, lon: Optional[float] = None) -> Optional[Dict[str, Any]]:
...
def fetch_forecast(city: Optional[str] = None, lat: Optional[float] = None, lon: Optional[float] = None) -> Dict[str, Any]:
    params = {"appid": OPENWEATHER_API_KEY, "units": "metric"}
    if city:
        params["q"] = city
    elif lat is not None and lon is not None:
...
def parse_current_weather(data: dict) -> Dict[str, Any]:
    ...
    sunrise_time = datetime.fromtimestamp(sunrise, tz=timezone.utc).time() if sunrise else None
    sunset_time = datetime.fromtimestamp(sunset, tz=timezone.utc).time() if sunset else None
